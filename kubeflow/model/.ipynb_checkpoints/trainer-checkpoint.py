...
#discriminator
def discriminator_block(layer_input, filters, kernel_size = 5, strides = 2, padding = 'same', activation='leakyrelu', dropout_rate=0.25, norm=True,name='layer'):
...
def build_discriminator():
    ...
    model = discriminator_block(discriminator_input, 32, 5, norm=False,name='layer1')
    model = discriminator_block(model, 64, 5, 2,name='layer2')
    model = discriminator_block(model, 64, 5, 2,name='layer3')
    model = discriminator_block(model, 64, 5, 2,name='layer4')
    model = discriminator_block(model, 64, 5, 2,name='layer5')
...
#                 tf.summary.image("Masked data", masked_images * 255.0,max_outputs=masked_images.shape[0], step=2)
#                 tf.summary.image("Training data", fake_images * 255.0,max_outputs=masked_images.shape[0], step=2)
            disc_loss_real = discriminator.train_on_batch(label, np.ones(len(label), dtype='float32'))
            disc_loss_fake = discriminator.train_on_batch(fake_images, np.zeros(len(fake_images), dtype='float32'))
  ...
  # generator.save(fn_args.serving_model_dir, save_format='tf', signatures=signatures)
